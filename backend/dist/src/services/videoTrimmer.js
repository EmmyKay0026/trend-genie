...
exports.trimAndUploadSegments = trimAndUploadSegments;
 ...
 */
async function trimAndUploadSegments(supabase, videoBuffer, segments) {
    const output = [];
    // Save the original file to temp
    const inputPath = path_1.default.join(__dirname, `input_${Date.now()}.mp4`);
    ...
    try {
        for (const seg of segments) {
            ...
            const outputPath = path_1.default.join(__dirname, clipName);
